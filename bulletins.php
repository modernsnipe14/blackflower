...
<?php
  if (isset($_GET["bulletin_id"])) {
    ...
    elseif (MYAFFROWS() > 1) {
      syslog(LOG_WARNING, "Updated too many (" . MYAFFROWS() . ") bulletin_views for id [$bulletin_id], user [". $_SESSION["id"]."]");
    }
  }
  ...
  if (isset($_GET["bulletin_id"])) {
    ...
    if (MYAFFROWS() != 1) {
      print "Error - expected 1 row, but got " . MYAFFROWS() . " rows for bulletin $bulletin_id ";
      syslog(LOG_WARNING, "Error - expected 1 row, but got " . MYAFFROWS() . " rows for bulletin $bulletin_id ");
      exit;
    }
